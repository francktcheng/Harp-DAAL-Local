...
{/*{{{*/
	...
	// daal::internal::RNGs<algorithmFPType, daal::sse2> rng(time(0));
	daal::internal::BaseRNGs<daal::sse2> base_rng;
	daal::internal::RNGs<algorithmFPType, daal::sse2> rng;
		...
		for (int64_t j=0;j<col_num_h;++j) 
		{
			if (i == j)
			{
				...
				if (counts_train < num_Train)
				{
                    ...
                    rng.uniform(1, &value, base_rng.getState(), 0.0, 1.0);
